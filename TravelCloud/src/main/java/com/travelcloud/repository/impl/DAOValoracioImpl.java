...
public class DAOValoracioImpl implements DAOValoracio{
	...
	public void insertarValoracio(Valoracio valoracio) throws Exception{
		...
		try {
			...
			pStatement.setInt(1, valoracio.getIdUSUARI().getIdUSUARI());
			pStatement.setInt(2, valoracio.getIdVIATGE().getIdVIATGE());
	...
	public void modificarValoracio(Valoracio valoracio) throws Exception{
		...
		try {
			...
			pStatement.setInt(3, valoracio.getIdUSUARI().getIdUSUARI());
			pStatement.setInt(4, valoracio.getIdVIATGE().getIdVIATGE());
			pStatement.executeUpdate();
			pStatement.close();
		} catch (Exception e) {
			throw e;
	...
	public List<Valoracio> llistarValoracionsPerViatge(int idViatgeValoracio) throws Exception{
		...
		try {
			...
			while(rs.next()) {
				...
				if (idViatgeValoracio == valoracio.getIdVIATGE().getIdVIATGE()) {
	...
	private Usuari makeValoracio(ResultSet rs) throws SQLException {
